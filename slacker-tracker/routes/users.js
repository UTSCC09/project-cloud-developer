...
router.post("/username", (req, res, next) => {
    ...
    UserModel.findOne({ email: req.body.email }, "-password", (err, user) => {
        if(err) return res.status(500).json({message: err});
        if(!user) return res.status(404).json({message: "user does not exist"});
        UserModel.updateOne({ email: req.body.email }, { username: req.body.username }, { upsert: true }, (err, data) => {
            if(err) return res.status(500).json({message: err});
...
router.post("/password", (req, res, next) => {
    ...
    if (req.body.new_password.length < 6) 
        return res.status(400).json({ message: "the length of the new password must be greater than 5" });
        ...
        bcrypt.compare(req.body.old_password, user.password, function (err, valid) {
            ...
            bcrypt.genSalt(10, function (err, salt) {
                ...
                bcrypt.hash(req.body.new_password, salt, function (err, hash) {
                    if(err) return res.status(500).json({message: err});
                    UserModel.updateOne({ email: req.body.email }, { password: hash }, { upsert: true }, (err, data) => {
                        if(err) return res.status(500).json({message: err});
                        return res.status(200).json({message: "success", data: data});
                    });
                });
...
router.post("/signup", (req, res, next) => {
    ...
    if (password.length < 6) 
        return res.status(400).json({ message: "the length of the password must be greater than 5" });
