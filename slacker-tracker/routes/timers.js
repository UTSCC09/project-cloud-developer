...
router.post(
  ...
  (req, res, next) => {
    ...
    if (!['work', 'play', 'study'].includes(req.body.dutyName)) { return res.status(400).json({ message: 'wrong duty name' }) }
...
router.post(
  ...
  (req, res, next) => {
    ...
    TimerModel.findOne({ _id: req.body._id }, (err, user) => {
      ...
      const newData = { duty: { name: 'unallocate', startTime: dutyStopTime } }
