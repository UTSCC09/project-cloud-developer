...
router.post(
  ...
  (req, res, next) => {
    ...
    if (req.body.dutyName === "")
      return res.status(400).json({ message: "Duty name is not defined" });
...
router.post(
  ...
  (req, res, next) => {
    ...
    TimerModel.findOne({ email: req.body.email }, (err, user) => {
      ...
      let duty = user.allocatedTime.find(
...
router.post(
  ...
  (req, res, next) => {
    ...
    TimerModel.findOne({ email: req.body.email }, (err, user) => {
      ...
      if (newUnallocatedTime < 0)
        return res.status(400).json("You don't have enough unallocated time");
  ...
  (req, res, next) => {
    ...
    TimerModel.findOne({ email: req.body.email }, (err, user) => {
      ...
      TimerModel.updateOne(
        { email: req.body.email },
        {
          duty: { name: duty.dutyName, startTime: startTime },
        },
        { upsert: true },
        (err, data) => {
          if (err) return res.status(500).json({ message: err });
