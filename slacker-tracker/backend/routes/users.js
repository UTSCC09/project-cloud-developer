...
router.post(
  ...
  (req, res, next) => {
    ...
    if (req.session.user._id !== req.body._id) { return res.status(401).json({ message: 'access denied' }) }
...
router.get('/signout', function (req, res, next) {
  ...
  TimerModel.findOne({ _id: req.session.user._id }, (err, userTimer) => {
    ...
    if (userTimer.duty.name === 'offline') { return res.status(400).json({ message: 'You have already logout' }) }
    const logoutTime = new Date()

    const newData = { duty: { name: 'offline', startTime: logoutTime } }
